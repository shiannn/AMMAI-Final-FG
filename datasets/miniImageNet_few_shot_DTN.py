...
class SetDataManager_DTN(DataManager):
    def __init__(self, image_size, mode='train', n_way=5, n_gen_pairs=5 , n_support=5, n_query=16, n_eposide = 300):        
    ...
    def get_generation_loader(self, aug):
        transform = self.trans_loader.get_composed_transform(aug)
        dataset = SetDataset(self.batch_size, transform, self.mode)
        generate_sampler = EpisodicBatchSampler(len(dataset), self.n_gen_pairs, self.n_eposide )
